...
export class GeminiService {
    ...
    static getApiKey(): string {
        return vscode.workspace.getConfiguration('workstash.ai').get<string>('geminiApiKey', '');
    }
    ...
    async generateContent(
        ...
        if (!apiKey) {
            throw new Error('Gemini API key not configured. Set it in Settings → WorkStash → AI → Gemini API Key.');
        }
